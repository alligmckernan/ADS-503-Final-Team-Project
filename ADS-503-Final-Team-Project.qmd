...
#Histogram 
hist.data.frame(heart_data[sapply(heart_data, is.numeric)])
# Histogram of all data features  

pairs(heart_data[sapply(heart_data, is.numeric)]) 
# pairwise scatter plots of predictor variables 
```
...
```{r}
#create training dataset for heart_data_scaled #80% 
#of data to the trianing dataset 
set.seed(seed) 
trainingRows <- createDataPartition(heart_data$HeartDisease, 
                                    p = .80, list = FALSE)
...
preproc <- preProcess(trainingData[, numeric_features], 
...
#Histogram 
hist.data.frame(heart_train[sapply(heart_train, is.numeric)]) 
# Histogram of all data features  

pairs(heart_train[sapply(heart_train, is.numeric)]) 
# pairwise scatter plots of predictor variables 
```
...
testResults <- data.frame(obs = heart_test$HeartDisease, 
                          LM = predict(log_model, heart_test[, 1:11]))
```
...
# Confusion matrix
conf_mat2 <- table(Predicted = xgb_model_predictions$pred, 
                   Actual = xgb_model_predictions$actual)
print(conf_mat2)
...
xgb_ROC <- roc(heart_test$HeartDisease, prob_predictions$Yes)  
# adjust if positive class is "Yes"
plot(xgb_ROC)
print(auc(xgb_ROC))
...
model3_predictions = data.frame(actual = heart_test$HeartDisease,
                                pred= predict(model3,heart_test))
...
model3_ROC=roc(as.numeric(model3_predictions$actual),
               as.numeric(model3_predictions$pred))
plot(model3_ROC)
...
```{r}
#Used ChatGPT to trouble shoot NA warning on SVM model
#https://chatgpt.com/share/6858305d-5ad8-800f-82e7-4ba1ca12d171
#Convert factor variabels to numeric codes for 
#sigma estimation 
x_numeric <- x 
x_numeric <- data.frame(lapply(x_numeric, function(col) {
  if (is.factor(col)) as.numeric(col) else col
}))

#Creat SVM grid 
sigmaEst <- kernlab::sigest(as.matrix(x_numeric))
svmgrid <- expand.grid(sigma = sigmaEst, C = 2^seq(-4,+4))

set.seed(seed)
svmModel <- train(x = x_numeric, y= y,
                 method = "svmRadial",
                 tuneGrid = svmgrid,
                 metric = "ROC",
                 trControl = ctrl)

#Create heart_test_numeric for model predictions
heart_test_numeric <- heart_test
#remove HeartDisease Column 
heart_test_numeric <- heart_test_numeric[, colnames(heart_test_numeric) != "HeartDisease"]


for(col in names(heart_test_numeric)) {
  if (is.factor(heart_test_numeric[[col]])) {
    # Match factor levels to training levels
    heart_test_numeric[[col]] <- factor(heart_test_numeric[[col]], levels = levels(x[[col]]))
    # Convert factor to numeric codes
    heart_test_numeric[[col]] <- as.numeric(heart_test_numeric[[col]])
  }
}

# Predict
testResults$svm <- predict(svmModel, heart_test_numeric)
```
...
#Support Vector Machines 
svmRoc <- roc(response = svmModel$pred$obs,
             predictor = svmModel$pred$No,
             levels = rev(levels(svmModel$pred$obs)))
#Decision Tree
...
plot(svmRoc, type = "s", add = TRUE, col = 'lightgreen', legacy.axes = TRUE)
legend("bottomright", legend=c("XGB", "NB", "SVM"),
       col=c("cyan","coral3", "lightgreen"), lwd=2)
...
cat("SVM accuracy:", accuracy(testResults$obs, testResults$svm), "\n")
...
#Yadav, A. (2024). SHAP Values in R. Medium. 
#https://medium.com/biased-algorithms/shap-values-in-r-c978a4c830c6
...
```{r}
#Used Chat GPT to figure out how to create a shiny app of a 
#Roc Curve https://chatgpt.com/share/685571d1-f978

#List of ROC objects 
roc_list <- list(
  "Logistic Regression" = lmRoc,
  "Ridge Logistic Regression" = rrRoc,
  "Lasso Logistic Regression" = lrRoc,
  "Elastic Net Logistic Regression" = enRoc,
  "XGBoost" = xgbRoc,
  "Naive Bayes" = nbRoc,
  "Support Vector Machine" = svmRoc,
  "Decision Tree" = dtRoc,
  "Random Forest" = rfRoc,
  "K-Nearest Neighbors" = knnRoc,
  "Neural Network" = nnRoc
)

#User Interface that creates a dropdown selection of 
#models used in this project.
ui <- fluidPage(
  titlePanel("ROC Curve Viewer"),
  sidebarLayout(
    sidebarPanel(
      selectInput("predictor", "Select Model:",
                  choices = names(roc_list)),
    ),
    mainPanel(
      plotOutput("rocPlot"),
...
#Server that filters data based on the user's choice of model, 
#then computes and displays the ROC and AUC.
server <- function(input, output) {
  
  roc_obj <- reactive({
    roc_list[[input$predictor]]
  })
  
  output$rocPlot <- renderPlot({
    plot(roc_obj(), type = "s", col = "blue", legacy.axes = TRUE,
         main = paste("ROC Curve for", input$predictor))
  })
